...
Real ANN Implementation on MNIST with Tensorflow
...
# Model hypothesis
ww_1 = tf.Variable(tf.zeros(dtype=tf.float32, shape=(n_features, n_hidden_units_1)), name = "MyWeights_1", trainable=True)
#bb_1 = tf.Variable(tf.zeros(dtype=tf.float32), shape=(n_hidden_units_1))
ww_2 = tf.Variable(tf.zeros(dtype=tf.float32, shape=(n_hidden_units_1, n_hidden_units_2)), name = "MyWeights_2", trainable=True)
#bb_2 = tf.Variable(tf.zeros(dtype=tf.float32), shape=(n_hidden_units_2))
ww_3 = tf.Variable(tf.zeros(dtype=tf.float32, shape=(n_hidden_units_2, n_outputs)), name = "MyWeights_final", trainable=True)
#bb_3 = tf.Variable(tf.zeros(dtype=tf.float32), shape=(n_hidden_units_3))
